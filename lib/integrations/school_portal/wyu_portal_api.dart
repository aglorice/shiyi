...
class WyuPortalApi {
  ...
  Future<Result<UserLogPage>> queryUserLogs(
    ...
    try {
      ...
      return FailureResult(
        NetworkFailure('网络异常，加载访问记录失败。',
            cause: error, stackTrace: stackTrace),
      );
    } catch (error, stackTrace) {
      return FailureResult(
        BusinessFailure('访问记录解析失败。',
            cause: error, stackTrace: stackTrace),
      );
    }
  }
  ...
  Future<Result<List<OnlineSession>>> queryOnlineSessions(
    ...
    try {
      ...
      return FailureResult(
        NetworkFailure('网络异常，加载当前登录失败。',
            cause: error, stackTrace: stackTrace),
      );
    } catch (error, stackTrace) {
      return FailureResult(
        BusinessFailure('当前登录解析失败。',
            cause: error, stackTrace: stackTrace),
      );
    }
  }
  ...
  String _labelFromPath(String path) {
    final tail = path.split('/').where((p) => p.isNotEmpty).toList();
    if (tail.isEmpty) return '';
    return '.${tail.last.replaceAll('.do', '')}';
  }
  ...
  Future<Result<dynamic>> _requestJxgl({
    ...
    try {
      ...
      _logger.debug('[JXGL] 发起教务请求 uri=$uri form=${_encodeForLog(formFields)}');
      final response = await _postFormWithHeaders(
        uri,
        formFields ?? const {},
        state.cookieStore,
  ...
  static const _gymServiceTypeId = '1296107574652694529'; // 生活服务
